...
def adset_performance_extract( adset_id ):
...
def performance_selection(adset_id, ad_id):
    mydb = connectDB("ad_activity")
    df_ad = pd.read_sql( "SELECT * FROM ad_insights WHERE adset_id=%s" % (adset_id), con=mydb )

    d={}
    ad_id_list = df_ad['ad_id'].unique().astype(int)
    if int(ad_id) in ad_id_list:
#         print('ad_id found')
        for ad in ad_id_list:
            d[ad] = df_ad['charge'][ df_ad.ad_id == ad ].sort_values().iloc[-1]
        biggestKey = max(d, key=d.get)
        adid = int(df_ad['charge'][ df_ad.ad_id == int(ad_id) ].iloc[-1])
...
if __name__=='__main__':
    performance_sort( 23843391009620564 )