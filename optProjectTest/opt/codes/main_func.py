...
#         mysql_adactivity_save.intoDB("campaign_target", df)
    ...
    def get_account_id( self ):
        ...
        account = campaign.get_insights(params=params,
                                        fields=[AdsInsights.Field.account_id])
        for acc in account:
...
def bid_adjust( campaign_id ):
    ...
    for ad_id in ad_id_list:
        ...
        df = selection.adset_performance_extract( adset_id )
        dfs = pd.concat([dfs, df], axis=0, sort=False)
    dfs = dfs.reset_index(drop=True)
    campaign_performance = dfs['charge'].sum()
    campaign_target = df_camp['target'].iloc[0] / campaign_days
    adset_target = campaign_target / adset_num
    for ad_id in ad_id_list:
        ad_id = ad_id.astype(dtype=object)
        center = 1
        width = 10
...
#         adset_performance = adset_target * time_progress
        ...
        print(campaign_id, ad_id, adset_performance, adset_time_target, adset_progress, df_camp['target'].iloc[0], bid)
...
def data_collect( campaign_id, total_clicks, charge_type ):
    ...
    lifetime_target['target'] = int(total_clicks) - charge
    campaign_dict =  {**Campaigns( campaign_id ).get_campaign_feature(), **charge_dict, **lifetime_target}
