...
FOLDER_PATH = '/storage/opt_project/opt/models/cpc_90/'
MODEL_PATH = FOLDER_PATH + 'IT_stateful_10_500_128.h5'
MODEL_SHAPE = 90
OUTPUT_SHAPE = 2
...
def decide_strategy(ad_id, cpc, clicks, impressions, reach, bid_amount, daily_budget):
    ...
    total_clicks = mysql_adactivity_save.getClicksData( ad_id )
    campaign_id = fb_graph.getCampIDbyAd( ad_id )
...
def concat_seq(seq, s):
    ...
    for i in range(0, PREDICT_HOUR_INTERVAL):
        ...
        seq_tmp = seq_tmp.append( tail.loc[[ COL_BID_AMOUNT, COL_DAILY_BUDGET]] )
