...
def check_campaignid_target(account_id, campaign_id, destination, charge_type, ai_start_date, ai_stop_date, ai_spend_cap, ai_status, custom_conversion_id, destination_max):
    mydb = connectDB(DATABASE)
    df = pd.read_sql( "SELECT * FROM campaign_target WHERE campaign_id=%s" % (campaign_id), con=mydb )   
    if df.empty:
        mycursor = mydb.cursor()
        sql = "INSERT INTO campaign_target ( account_id, campaign_id, destination, charge_type, ai_start_date, ai_stop_date, ai_spend_cap, ai_status, custom_conversion_id, destination_max ) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )"
        val = ( account_id, campaign_id, destination, charge_type, ai_start_date, ai_stop_date, ai_spend_cap, ai_status, custom_conversion_id, destination_max )
        mycursor.execute(sql, val)
        mydb.commit()
        mycursor.close()
        mydb.close()
        ...
        sql = "UPDATE campaign_target SET account_id=%s, destination=%s, charge_type=%s, ai_start_date=%s, ai_stop_date=%s, ai_spend_cap=%s, ai_status=%s,custom_conversion_id=%s, destination_max=%s WHERE campaign_id=%s"
        val = ( account_id, destination, charge_type, ai_start_date, ai_stop_date, ai_spend_cap, ai_status, custom_conversion_id, destination_max, campaign_id )
        mycursor = mydb.cursor()
        mycursor.execute(sql, val)
        mydb.commit()
        mycursor.close()
